...
function LiveTranscriptStrip({
  ...
  const display = streamed || (listening ? "Speak now" : "");
  const isPlaceholder = !text || display === "Speak now";
  ...
  return (
    <div className="relative min-w-0 flex-1 overflow-hidden pl-2">
      ...
      />
      <p
        className={`truncate whitespace-nowrap text-[14px] font-medium leading-snug ${
...
export function getSuggestionReadDurationMs(text: string): number {
  ...
  return Math.min(14000, Math.max(4500, 2800 + words * 380));
}
      ...
      <div className="px-4 py-3">
        <div className="mb-1.5 flex items-center justify-between gap-2">
          ...
          {dealHealth != null && !loading ? (
            <p className={`text-[11px] font-semibold ${healthColor}`}>Deal health {dealHealth}</p>
          ) : null}
        </div>
        {loading && !suggestion ? (
