...
SALES CALL (live rep on a call):
ONE line under 20 words — exact words the rep says verbatim. Plain text only, no bullets, no code fences.
...
export function buildCopilotSystemPrompt(
  ...
  let prompt = `You are Replii, a real-time meeting copilot in a live overlay.

The user may be on sales calls, interviews, technical screens, or general meetings.
Rep sells (when relevant): ${product}
...
${GLOBAL_RULES}`;
  ...
  if (coachingContext?.trim()) {
    ...
    const uploadHint = usesUploads
      ? "\nGround sales answers in UPLOADED PLAYBOOKS when product, pricing, or objection facts are needed."
      : "";
...
export function hintFormatReminder(hint: ContentHint): string {
  switch (hint) {
    case "behavioural":
      return "Format: BEHAVIOURAL / INTERVIEW template.";
    case "coding":
    ...
    case "sales":
      return "Format: SALES CALL — one line under 20 words, verbatim script.";
    default:
