...
if (useP12) {
  ...
  console.log("[replii] Using Developer ID certificate from Keychain (no .p12 needed).\n");
} else if (existsSync(process.env.CSC_LINK?.trim() ?? "")) {
  console.error("[replii] .p12 found but CSC_KEY_PASSWORD is wrong or missing.");
  console.error("[replii] Fix CSC_KEY_PASSWORD in .release-secrets.local, or re-export from Keychain.\n");
}
...
if (missing.length) {
  console.error("[replii] Missing release signing secrets.\n");
...
if (useP12 && p12 && !existsSync(p12)) {
  console.error(`[replii] Certificate file not found: ${p12}`);
  console.error("[replii] Check CSC_LINK in .release-secrets.local (Desktop filename may differ).");
  process.exit(1);
}

console.log("[replii] Building signed + notarized Replii.dmg (secrets stay local — not committed).\n");

const result = spawnSync("npm", ["run", "package:mac"], {
  cwd: desktopRoot,
  stdio: "inherit",
  env: { ...process.env, REPLII_RELEASE_SIGN: "1" },
  shell: true,
});
