...
export function DashboardTopBar({
  ...
  const isAdminPage = isAdminRoute && isAdminUser(user?.email);
  const canGoBack = isAdminRoute || location.pathname !== "/";
  ...
  return (
    <header className="no-drag relative z-20 flex h-[52px] shrink-0 items-center border-b border-zinc-100 bg-white px-4 pr-4">
      <div className="drag-region absolute inset-y-0 left-0 w-[72px]" aria-hidden />

      {isAdminRoute ? (
        <div className="no-drag flex w-full items-center gap-2 pl-[72px]">
          <button
            type="button"
            onClick={handleBack}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-zinc-400 transition-colors hover:bg-zinc-200/60 hover:text-zinc-700"
          ...
          {isAdminPage ? (
            <h1 className="text-[14px] font-semibold text-zinc-900">Admin Dashboard</h1>
          ) : null}
