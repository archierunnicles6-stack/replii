...
> = {
  accessibility: {
    title: "Accessibility",
    description:
      "Lets Replii appear on any call with one hotkey and control the overlay.",
    settingsLabel: "Open Accessibility Settings",
  },
  microphone: {
    title: "Microphone",
    description:
      "Replii listens to your calls to coach you in real time. Audio stays on your device.",
    settingsLabel: "Open Microphone Settings",
  },
};

export function SessionPermissionsModal() {
  const open = useAppStore((s) => s.sessionPermissionsOpen);
  const closeSessionPermissions = useAppStore((s) => s.closeSessionPermissions);
  ...
  useEffect(() => {
    ...
    const unsub = window.replii?.onPermissionsChanged?.(onFocus);
    const unsubMic = window.replii?.onMicGranted?.(onFocus);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (started) closeSessionPermissions();
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-6">
      <div
        ...
        onClick={closeSessionPermissions}
      />
      ...
      >
        ...
        >
          Allow Replii on your Mac
        </h2>
        <p className="mt-2 text-[14px] leading-relaxed text-zinc-500">
          macOS needs a couple of permissions before Replii can join your calls.
        </p>

        <ul className="mt-5 space-y-4">
          {(["accessibility", "microphone"] as const).map((key) => {
