...
export function buildCoachingInsightsContext(
  ...
  const lines = top.map(
    (t) =>
      `${t.label} suggestions (${Math.round(t.winRate * 100)}% on won deals, n=${t.count})`,
  );

  return `Coaching patterns from your closed deals: ${lines.join("; ")}. Prefer approaches aligned with these winning patterns when relevant.`;
}
