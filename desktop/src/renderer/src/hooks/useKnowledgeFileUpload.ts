...
export function useKnowledgeFileUpload(options?: {
  ...
  const maxDocsMessage =
    ...
    `Maximum ${MAX_KNOWLEDGE_DOCS} files. Remove one to add another.`;
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const added = addKnowledgeDocument(file.name, text);
        if (!added) {
          setUploadError(maxDocsMessage);
        }
      } catch (err) {
    ...
    [addKnowledgeDocument, knowledgeFiles.length, maxDocsMessage],
  );
